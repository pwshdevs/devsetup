BeforeAll {
    . $PSScriptRoot\Uninstall-ChocolateyPackages.ps1
...
Describe "Uninstall-ChocolateyPackages" {

    Context "When not running as administrator" {
        It "Should throw and return false" {
            Mock Test-RunningAsAdmin { $false }
            $result = Uninstall-ChocolateyPackages -YamlData @{ devsetup = @{ dependencies = @{ chocolatey = @{ packages = @("git") } } } } 
    ...
    Context "When Chocolatey packages config is missing" {
        It "Should write warning and return" {
            ...
            $result = Uninstall-ChocolateyPackages -YamlData $yamlData
    ...
    Context "When all packages uninstall successfully (string format)" {
        It "Should process all packages and return true" {
            ...
            #Assert-MockCalled Write-StatusMessage -Scope It -ParameterFilter { $Object -match "uninstallation completed" }
        }
    }

    Context "When all packages uninstall successfully (object format)" {
        It "Should process all packages and return true" {
            $yamlData = @{
                devsetup = @{
                    dependencies = @{
    ...
    Context "When some packages fail to uninstall" {
    ...
    Context "When an exception occurs during uninstallation" {
        It "Should write error and return false" {
            Mock Uninstall-ChocolateyPackage { throw "Unexpected error" }
            $yamlData = @{
                devsetup = @{
                    dependencies = @{
                        chocolatey = @{
            ...
            $result = Uninstall-ChocolateyPackages -YamlData $yamlData
            $result | Should -Be $false
            Assert-MockCalled Write-Error -Scope It -ParameterFilter { $Message -match "Error uninstalling Chocolatey packages" }
        }
    }
}